...
window.liquidElements = ($) => {
  ...
  if (liquidIsElementor) {
    ...
    window.$liquidSectionsWrapper = $secWrap.legth
      ? $secWrap
      : $("> .elementor", $liquidContentsWrap).first()
  }
  if (isPfSinglePage) {
    window.$liquidSectionsWrapper = $(".pf-single-contents")
    const $elementorWrapper =
      window.$liquidSectionsWrapper.children(".elementor")
  ...
  const $elementorFooterSections = $("> .elementor", $liquidMainFooter).find(
...
class LiquidSectionsDetails {
  constructor() {
    this.sections = []
    this.footerBg =
      tinycolor(liquidMainFooterBg).getAlpha() === 0
        ? liquidBodyBg
        : liquidMainFooterBg
  }
  static getInstance() {
  ...
  createDetailsObj(liquidContentsRect, rowRect, DOM, isLiquidContentElement) {
    return new Promise((resolve) => {
      fastdom.measure(async () => {
        ...
        const elementorTopContainer = DOM.$element.parents(
          ".e-container, .e-con"
        )
        obj.borderColor = styles.borderColor
        obj.isOuterSection = liquidIsElementor
          ? DOM.element.classList.contains("elementor-top-section") ||
            !!!elementorTopContainer.length
          : DOM.element.classList.contains("vc_section") ||
            DOM.element.parentElement.closest(".lqd-section") == null
        obj.isInnerSection = liquidIsElementor
          ? DOM.element.classList.contains("elementor-inner-section") ||
            !!elementorTopContainer.length
          : DOM.parent.classList.contains("vc_section") ||
            DOM.element.parentElement.closest(".lqd-section") != null
        ...
        if (obj.isInFooter) {
          ...
          if (footerParent.hasAttribute("data-sticky-footer")) {
            ...
            const footerHeight =
              ...
              (liquidContentsRect.y + scrollY) -
              liquidContentsRect.height
            const elPositionTop = Math.abs(
              window.innerHeight - footerHeight - obj.rect.y
            )
  ...
  addParentSections(instance) {
    ...
    innerSections.forEach((innerSec) => {
      ...
      if (liquidIsElementor) {
        parentSec =
          innerSec.el.closest(".elementor-top-section") ||
          innerSec.$el.parents(".e-container") ||
          innerSec.$el.parents(".e-con")
      } else {
        parentSec =
          innerSec.el.closest(".vc_section") ||
          innerSec.el.parentElement.closest(".lqd-section")
      }
  ...
  addInnerSections(instance) {
    ...
    instance.sections.forEach((outerSec, i) => {
      ...
      innerSections.forEach((innerSec) => {
        if (
          innerSec.parentSection &&
          innerSec.parentSection.el === outerSec.el
        ) {
      ...
      $(this.animationTargets).each((i, targetElement) => {
        ...
        if (targetElement.hasAttribute("data-animations")) {
          ...
          timeline.fromTo(
            targetElement,
            {
              [Object.keys(animationProperties)[0]]:
                Object.values(animationProperties)[0][0],
            },
            {
              [Object.keys(animationProperties)[0]]:
                Object.values(animationProperties)[0][1],
            },
            stagger * i
          )
        }
...
;(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      this.$elementorHeaderWidgets =
        liquidIsElementor &&
        $liquidMainHeader
          .find(
            "> .elementor:not(.lqd-mobile-sec) > .elementor-section-wrap > .elementor-section, > .elementor:not(.lqd-mobile-sec) > .elementor-section, > .elementor:not(.lqd-mobile-sec) > .e-con"
          )
          .not(".lqd-stickybar-wrap")
          .find(
            '> .elementor-container > .elementor-column > .elementor-widget-wrap > [data-element_type="widget"]'
          )
      this.reactToMegamenuEnabled =
        $liquidMainHeader.attr("data-react-to-megamenu") === "true"
      this.init()
    }
    init() {
      ...
      this.$submenuParents.each(async (i, subParent) => {
        ...
        if (
          (!liquidIsMobile() || !$(subParent).is(":hidden")) &&
          !this.itemsAreFullwidth
        ) {
          if (!isMegamenu) {
    ...
    handleToggle(state, event) {
      ...
      const megamenuScheme =
        isMegamenu && $submenuParent.attr("data-megamenu-bg-scheme")
      if (!$submenu.length) return
      event.preventDefault()
      const stateIsShow =
        state === "show" || !$submenuParent.hasClass("is-active")
      if (toggleType === "fade") {
        if (state === "show") {
      ...
      $submenuParent
        .toggleClass(
          `is-active ${handler === "mouse-in-out" ? "is-hovered" : ""}`,
          stateIsShow
        )
        .siblings()
        .removeClass(
      ...
      document.dispatchEvent(
        new CustomEvent("lqd-submenu-change", {
          bubbles: false,
          detail: {
    ...
    fadeToggle(submenu, isMegamenu, megamenuScheme, megamenuBg) {
      ...
      if (this.reactToMegamenuEnabled) {
        ...
        $liquidMainHeader.toggleClass(
          `megamenu-scheme-${megamenuScheme}`,
          isMegamenu
        )
        if (
          this.$elementorHeaderWidgets &&
          megamenuScheme !== "transparent" &&
          !$liquidMainHeader.hasClass("is-stuck")
        ) {
          this.$elementorHeaderWidgets.removeClass(
            `lqd-active-row-dark lqd-active-row-light`
          )
          this.$elementorHeaderWidgets.addClass(
            `lqd-active-row-${megamenuScheme}`
          )
        }
      }
    ...
    getMegamenuBackgroundLuminance(subParent) {
      const megamenuRowsWrap = subParent.querySelector(
      ...
      fastdom.mutate(() => {
        ...
        if (backgroundColor.getAlpha() === 0) {
          return subParent.setAttribute(
            "data-megamenu-bg-scheme",
            "transparent"
          )
        }
    ...
    closeActiveSubmenu(event) {
      ...
      if (event.keyCode) {
        this.$submenuParents.each((i, submenuParent) => {
          ...
          this[toggleType === "fade" ? "fadeOut" : "slideUp"](
            $submenuParent.children(".nav-item-children, .children"),
            $submenuParent.hasClass("megamenu")
          )
        })
        document.dispatchEvent(
          new CustomEvent("lqd-submenu-change", {
    ...
    positioning(i, submenu, submenuParent) {
      return fastdomPromised.mutate(() => {
        ...
        submenuParent.style.setProperty(
          "--item-height",
          `${subParentRect.height}px`
        )
  ...
  class Plugin {
    constructor(element, options) {
      ...
      this.isModernMobileNav =
        ...
        document.body.getAttribute("data-mobile-nav-style") === "modern"
      this.$navItems = []
      this.init()
    }
    init() {
    ...
    mobileExpander() {
      ...
      this.$mobileNavCollapse
        ...
        .add(".nav-item-children", $(".ld-sd-inner"))
        .css({
          display: "block",
          visibility: "hidden",
        })
    ...
    eventHandlers() {
      this.$submenuExpanders.off().on("click keypress", (event) => {
        ...
        $submenu.stop().slideToggle(300, () => {
          if (
            this.isModernMobileNav &&
            !submenuParentWasActive &&
            $navbarInner.length
          ) {
            $navbarInner.animate({
              scrollTop:
                $navbarInner.scrollTop() +
                ($parentLi.offset().top - $navbarInner.offset().top),
            })
          }
        })
      })
...
;(function ($) {
  ...
  class Plugin {
    ...
    gradientBorderRoundness() {
      ...
      if (
        element.find(".btn-gradient-border").length &&
        element.hasClass("circle") &&
        element.is(":visible")
      ) {
...
;(function ($) {
  ...
  Plugin.prototype = {
    ...
    resizer() {
      ...
      const elementWidth = $element.parent(".ld-fancy-heading").length
        ? $element.parent().width()
        : $element.width()
      $element.css(
        "font-size",
        Math.max(
          Math.min(elementWidth / (compressor * 10), parseFloat(maxFontSize)),
  ...
  class Plugin {
    ...
    initIO() {
      ...
      new IntersectionObserver(
              ...
              fastdom.mutate(() => {
                if (toggleBehavior === "toggleInView") {
                  if (
                    ...
                    boundingClientRect.top + scrollY
                  ) {
                    this.isVisible = true
                    this.callFns()
                ...
                if (entry.isIntersecting && toggleBehavior === "stay") {
                  ...
                  toggleBehavior === "toggleOutOfView"
                ) {
                  this.onOutOfView()
                }
              })
        ...
        {
          threshold:
            toggleBehavior === "toggleInView" ? [0, 0.25, 0.5, 0.75, 1] : [0],
        }
...
;(function ($) {
  ...
  class Plugin {
    ...
    getCurrentSrc() {
      let imageSrc = this.targetImage.currentSrc
    ...
    reInitparallaxBG() {
      const parallaxFigure = this.$element
...
;(function ($) {
  ...
  const contentsIsLiquidBg =
    $liquidContents.length &&
    $liquidContents[0].getAttribute("data-liquid-bg-options")
  const contentsInteractWithHeader =
    contentsIsLiquidBg &&
    (JSON.parse(contentsIsLiquidBg).interactWithHeader === true ||
      JSON.parse(contentsIsLiquidBg).interactWithHeader === "true")
  ...
  class Plugin {
    constructor(element, options) {
      ...
      this.DOM.$stickySections = liquidIsElementor
        ...
        : $(".lqd-head-sec-wrap", element).not(".lqd-hide-onstuck")
      this.isInTitlebar =
      ...
      this.firstRowIsSticky = this.firstRow
        ? getComputedStyle(this.firstRow).position === "sticky"
        : false
    ...
    addStickySectionsHeight() {
      const applyCssTo = liquidIsElementor
        ? document.body
        : document.documentElement
      fastdomPromised.mutate(() => {
        applyCssTo.style.setProperty(
    ...
    addSentinel() {
      return fastdomPromised.mutate(() => {
        const sentinelTemplate = document.querySelector(
          "#lqd-temp-sticky-header-sentinel"
        )
        const sentinel =
          sentinelTemplate.content.firstElementChild.cloneNode(true)
      ...
      fastdomPromised
        .measure(() => {
          ...
          const scrollDirection =
            scrollY > this.smartStickyStuff.prevScrollY ? "down" : "up"
          return {
            scrollY,
            scrollDirection,
          }
        ...
        .then(({ scrollY, scrollDirection }) => {
          this.smartStickyStuff.scrolledDistance = Math.abs(
            scrollY - this.smartStickyStuff.prevScrollY
          )
          fastdom.mutate(() => {
            if (this.isStuck && this.smartStickyStuff.toleranceExceeded) {
              if (
                scrollDirection === "up" &&
                this.smartStickyStuff.state !== "show"
              ) {
                this.stickyVisibilityChange("show")
              } else if (
                scrollDirection === "down" &&
                this.smartStickyStuff.state !== "hide"
              ) {
            ...
            this.smartStickyStuff.toleranceExceeded =
              ...
              this.smartStickyStuff.tolerance[scrollDirection]
          })
        })
    }
    ...
    getStickyElements() {
      ...
      const $elements = this.DOM.element.hasAttribute("data-liquid-bg")
      ...
      this.DOM.$stickyElements.each((i, stickyEl) => {
        const promise = new Promise((resolve) => {
          new IntersectionObserver(([entry], observer) => {
            ...
            stickyEl === this.DOM.element ? this.DOM.element : stickyEl
          )
        })
        promises.push(promise)
      })
    ...
    getSections(visibleSections) {
      ...
      visibleSections.forEach((sec) => {
        ...
        if (sec.isInnerSection) {
          ...
          if (!section) return
          const sectionBgcolor = sec.backgroundColor
            .replace(/, /g, ",")
            .split(" ")[0]
          const parentBgColor = section.backgroundColor
            .replace(/, /g, ",")
            .split(" ")[0]
          const sectionBgAlpha = tinycolor(sectionBgcolor).getAlpha()
          const parentBgAlpha = tinycolor(parentBgColor).getAlpha()
          if (
            (sectionBgAlpha !== 0 || sec.predefinedLuminosity) &&
            parentBgAlpha === 0
          ) {
            section = sec
            sections = sections.filter(
              (sect) => sect.el !== section.parentSection.el
            )
          }
        }
    ...
    initDynamicColors() {
      ...
      LiquidSectionsDetails.getDetails().then((lqdSections) => {
        ...
        const onscroll = liquidThrottle(
          this.onScroll.bind(this, sections),
          150,
          {
            leading: true,
          }
    ...
    onScroll(sections) {
      for (
        let sectionsItterator = 0;
        sectionsItterator < sections.length;
        sectionsItterator++
      ) {
        fastdomPromised.measure(() => {
          ...
          for (
            let stickyItterator = 0;
            stickyItterator < this.stickyElsDetails.length;
            stickyItterator++
          ) {
            if (
              this.isCollide(this.stickyElsDetails[stickyItterator].rect, rect)
            ) {
...
jQuery(document).ready(function ($) {
  ...
  if ($liquidContents.length) {
    ...
    if (
      ...
      (!pageStackEnabled ||
        (pageStackEnabled && liquidIsMobile() && pageStackDisabledOnMobile))
    ) {
...
;(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      this.activeEl = null
      const cssVarEl = liquidIsElementor
        ? document.body
        : document.documentElement
      const outerSize =
        window.liquidParams?.ccOuterSize ||
        getComputedStyle(cssVarEl).getPropertyValue("--lqd-cc-size-outer")
      this.outerCursorSize = parseInt(outerSize || 0, 10)
      this.activeCircleBg =
        window.liquidParams?.ccActiveCircleBg ||
        getComputedStyle(document.body).getPropertyValue("--lqd-cc-active-bg")
      this.activeCircleBc =
        window.liquidParams?.ccActiveCircleBc ||
        getComputedStyle(document.body).getPropertyValue("--lqd-cc-active-bc")
      ...
      document.addEventListener("lqd-carousel-initialized", (e) => {
        ...
        if (flkty.nextButton) {
          this.initCarouselNavCursor([
            flkty.nextButton.element,
            flkty.prevButton.element,
          ])
        }
        ...
        flkty.on("pointerDown", () => {
          this.cursorEls.forEach((el) =>
            el.classList.add("lqd-carousel-pointer-down")
          )
        })
        flkty.on("pointerUp", () => {
          this.cursorEls.forEach((el) =>
            el.classList.remove("lqd-carousel-pointer-down")
          )
        })
    ...
    initCarouselNavCursor(navElementsArray) {
      const carouselNavEnter = (navEl, navElStyle) => {
        ...
        this.moveOtherElements(
          navEl,
          [...navEl.querySelectorAll("i"), ...navEl.querySelectorAll("svg")],
          true
        )
      }
        ...
        fastdomPromised
          ...
          .then(({ navElStyle }) => {
            item.addEventListener(
              "mouseenter",
              carouselNavEnter.bind(this, item, navElStyle)
            )
    ...
    initExtraCursor() {
      ...
      this.extraCursors.forEach((item) => {
        ...
        itemParent.addEventListener(
          "mousemove",
          extraCursorEnter.bind(this, item)
        )
        itemParent.addEventListener(
          "mouseleave",
          extraCursorLeave.bind(this, item)
        )
      })
    }
    ...
    initButtonShapeCursors() {
      const buttonShapeEnter = (icon, iconStyles) => {
        ...
        this.moveOtherElements(icon, [
          ...icon.querySelectorAll("i"),
          ...icon.querySelectorAll("svg"),
        ])
      }
      ...
      const buttonWithIcon = document.querySelectorAll(
        ".btn-icon-bordered, .btn-icon-solid"
      )
        ...
        fastdomPromised
          ...
          .then(({ iconStyles }) => {
            btn.addEventListener(
              "mouseenter",
              buttonShapeEnter.bind(this, icon, iconStyles)
            )
      ...
      navTriggers.forEach((navTrigger) => {
        ...
        if (toggleData && toggleData.options.type !== "hover") {
          navTrigger.addEventListener(
            "mouseenter",
            navTriggerEnter.bind(this, bars, barsInner, true)
          )
      ...
      moduleTriggers.forEach((moduleTrigger) => {
        ...
        if (toggleData && toggleData.options.type !== "hover") {
          moduleTrigger.addEventListener("mouseenter", () => {
            const scaleOuterCursor =
              !moduleTrigger.classList.contains("lqd-module-icon-plain") &&
              !moduleTrigger.parentElement.classList.contains(
                "ld-dropdown-menu"
              )
      ...
      magneticMenuItems.forEach((el) => {
        ...
        menuItems.forEach((menuItem) => {
          menuItem.addEventListener(
            "mouseenter",
            menuItemEnter.bind(this, menuItem)
          )
    ...
    moveOtherElements(movingEl, innerMovingEl, onlyMoveInnerEl) {
      ...
      $liquidWindow.on("mousemove.lqdCCMoveOthers", (e) => {
        ...
        this.magneticCords = {
          x:
            movingElRect.left +
            movingElRect.width / 2 -
            this.outerCursorSize / 2 -
            (Math.sin(angle) * hypotenuse) / 3,
          y:
            movingElRect.top +
            movingElRect.height / 2 -
            this.outerCursorSize / 2 -
            (Math.cos(angle) * hypotenuse) / 3,
        }
        !onlyMoveInnerEl &&
          gsap.to(movingEl, {
            duration: 0.4,
  ...
  Plugin.prototype = {
    ...
    setHashOnLoad() {
      ...
      if (location.hash !== "" && element.find(location.hash).length) {
        const activeItemParent = element
          .find(location.hash)
  ...
  class Plugin {
    ...
    loadItems(event) {
      ...
      const nextPageNumber = parseInt(
        nextPageUrl.split("page/")[1].split("/")[0],
        10
      )
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (!$nextPageTrigger.length) {
            $nextPageTrigger = $newItemsWrapper
              .siblings(".ld-pf-nav-ajax")
              .find("[data-ajaxify=true]")
          }
          let newNextPageUrl = $nextPageTrigger.attr("href")
          const newNextPageNumber =
            newNextPageUrl &&
            parseInt(newNextPageUrl.split("page/")[1].split("/")[0], 10)
          if (newNextPageNumber <= nextPageNumber) {
            newNextPageUrl = nextPageUrl.replace(
              `page/${nextPageNumber}`,
    ...
    onSuccess($wrapper) {
      ...
      $("[data-custom-animations]", $wrapper).map((i, element) => {
        ...
        const $customAnimationParent = $element.parents(
          ".wpb_wrapper[data-custom-animations]"
        )
      ...
      $("[data-custom-animations]", $wrapper)
        .filter((i, element) => {
          const $element = $(element)
          const $rowBgparent = $element.closest(".vc_row[data-row-bg]")
          const $slideshowBgParent = $element.closest(
            ".vc_row[data-slideshow-bg]"
          )
...
;(function ($) {
  ...
  class Plugin {
    ...
    initNumbers() {
      ...
      const $parantSpan = $(
        '<span class="pos-abs pos-tl absolute top-0 left-0" />'
      )
      this.DOM.numsTotal.innerText = this.slidesTotal
      for (let i = 1; i <= this.slidesTotal; i++) {
        const $span = $(
          `<span class="d-flex align-items-center justify-content-center flex items-center justify-center">${i}</span>`
        )
    ...
    initEvents() {
      ...
      $(this.DOM.nextCtrl).on(
        ...
        this.navigate.bind(this, "next", true)
      )
      $(this.DOM.prevCtrl).on(
        "click.lqdAnimateFrames",
        this.navigate.bind(this, "prev", true)
      )
    ...
    navigate(dir = "next", navigatedByUser) {
      ...
      const currentSlideImgInner = currentSlideImg.querySelector(
        ".lqd-af-slide__img__inner"
      )
      const currentSlideFigureEl = currentSlideImg.querySelector("figure")
      const currentSlideTitleSplitInner = currentSlide.querySelectorAll(
        ".lqd-af-slide__title .split-inner"
      )
      const currentSlideDescSplitInner = currentSlide.querySelectorAll(
      ...
      figureAnimations.fromTo(
        currentSlideFigureEl,
        {
          scale: 1,
        },
        {
      ...
      currentSlideTitleSplitInner?.length &&
        currentSlideContentTimeline.fromTo(
          currentSlideTitleSplitInner,
          {
            y: "0%",
          },
          {
      ...
      currentSlideDescSplitInner?.length &&
        currentSlideContentTimeline.fromTo(
          currentSlideDescSplitInner,
          {
            opacity: 1,
            y: "0%",
          },
      ...
      const newSlideTitleSplitInner = newSlide.querySelectorAll(
        ".lqd-af-slide__title .split-inner"
      )
      const newSlideDescSplitInner = newSlide.querySelectorAll(
        ".lqd-af-slide__desc .split-inner"
      )
      ...
      newSlideTitleSplitInner?.length &&
        newSlideContent.fromTo(
          newSlideTitleSplitInner,
          {
            y: dir === "next" ? "105%" : "-105%",
          },
          {
      ...
      newSlideDescSplitInner?.length &&
        newSlideContent.fromTo(
          newSlideDescSplitInner,
          {
            opacity: 0,
            y: dir === "next" ? "100%" : "-100%",
          },
...
;(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      const promises = []
      const $firstHeading = $(this.DOM.titlesWrap)
        .find(".lqd-asym-slider-title-element")
        .first()
      const elementFontFamily = $firstHeading
        .css("font-family")
        .replace(/"/g, "")
        .replace(/'/g, "")
        .split(",")[0]
      ...
      promises.push(
        new Promise((resolve) => imagesLoaded(this.element, resolve))
      )
    ...
    events() {
      ...
      window.addEventListener(
        "resize",
        liquidDebounce(this.updateHeights.bind(this), 1000)
      )
    }
    updateHeights() {
    ...
    playTitle(dir) {
      ...
      const currentTitleChars = this.isRTL
        ? currentTitle.querySelectorAll(".lqd-words")
        : currentTitle.querySelectorAll(".lqd-chars")
      const nextTitleChars = this.isRTL
        ? nextTitle.querySelectorAll(".lqd-words")
        : nextTitle.querySelectorAll(".lqd-chars")
      const timeline = gsap.timeline({
        defaults: {
          duration: 1,
        },
    ...
    playInfo(dir) {
      ...
      const currentInfoTitle = currentInfo.querySelector(
        ".lqd-asym-slider-subtitle-element"
      )
      const currentInfoP = currentInfo.querySelector(
      ...
      const nextInfoTitle = nextInfo.querySelector(
        ".lqd-asym-slider-subtitle-element"
      )
      const nextInfoP = nextInfo.querySelector(
        ".lqd-asym-slider-description-element"
      )
      const timeline = gsap.timeline({
        defaults: {
    ...
    playImages(dir) {
      ...
      const currentImageInner = currentImage.querySelector(
        ".lqd-asym-slider-img-inner"
      )
      const nextImage = this.DOM.images[this.nextSlide]
      const nextImageInner = nextImage.querySelector(
        ".lqd-asym-slider-img-inner"
      )
      const timeline = gsap.timeline({
        defaults: {
...
;(function ($) {
  ...
  class Plugin {
    ...
    firstSectionIO() {
      let $firstSection = $liquidSectionsWrapper
        .children()
        .not("style, p")
        .first()
      ...
      new IntersectionObserver(
        ([entry]) => {
          ...
          if (firstSectionIsOnlyChild) {
            activate =
              rootBounds &&
              rootBounds.bottom - rootBounds.height / 2 >=
  ...
  class Plugin {
      ...
      sections.forEach(async (sec, i) => {
        ...
        Promise.all(this._getRects(sec, i)).then(() => {
          ...
          if (this.$bgEl) {
            ...
            sec.isInnerSection &&
              sec.parentSection &&
              sec.parentSection.el.classList.add("bg-transparent")
          }
        })
      })
    ...
    _getStickyModules() {
      ...
      if (
        ...
        if (liquidIsElementor) {
          stickyModules = $liquidMainHeader
            .find("> .elementor")
            .find(
    ...
    _addBgElement() {
      ...
      const heightClassname = liquidIsMobile()
        ...
        : "h-vh-100 h-100vh"
      this.$bgEl = $(
        `<div class="lqd-liquid-bg-el-wrap lqd-overlay pointer-events-none z-index--1 ${overflowClassname}"><div class="lqd-liquid-bg-el ${posClassname} pos-tl w-100 top-0 left-0 w-full ${heightClassname} pointer-events-none"></div></div>`
      )
    ...
    _getColors(lqdSection) {
      return fastdomPromised.measure(() => {
        ...
        if (lqdSection.isInnerSection && lqdSection.parentSection) {
          if (
            lqdSection.isBgTransparent &&
            !lqdSection.parentSection.isBgTransparent
          ) {
            color = lqdSection.parentSection[getBgFromSelector]
              .replace(/, /g, ",")
              .split(" ")[0]
          }
        }
    ...
    _interactWithHeader(colorLuminosity) {
      const $elements = $liquidMainHeader
        .add(this.$stickyModules)
        .filter((i, el) => {
          const $el = $(el)
          return (
            !$el.children(".navbar-brand-solid").length &&
            !$el.hasClass("navbar-brand-solid") &&
            !$el.find("> .elementor-widget-container > .navbar-brand-solid")
              .length
          )
        })
    ...
    _interactWithColors(element, targetColorLuminosity) {
      ...
      if ($el.hasClass("navbar-brand-inner")) {
        const $parentModule = liquidIsElementor
          ? $el.closest(".elementor-element")
          : $el.closest(".header-module")
    ...
    _getRects(lqdSection, loopIndex) {
      ...
      if (liquidIsElementor) {
        if (
          lqdSection.el.classList.contains("e-container") ||
          lqdSection.el.classList.contains("e-con")
        ) {
          row = lqdSection.el
        } else {
...
;(function ($) {
  ...
  class Plugin {
    ...
    animateIcon() {
      ...
      const gid =
        this.$element.attr("id") || Math.round(Math.random() * 1000000)
      const containerEl = this.$element
        .find(objContainer)
        .attr("id", `${gid.replace(/ld_icon_box/, "ld_icon_container")}`)
    ...
    onReady(vivus) {
      ...
      this.animate(vivus)
      if (
        $parentCarousel.length &&
        carouselData != null &&
        carouselData.flickityData != null
      ) {
    ...
    addColors(svg) {
      ...
      let strokeHoverGradients = document.createElementNS(
      ...
      strokegradients =
        ...
        "</linearGradient>" +
        "</defs>"
      const xmlStrokegradients = new DOMParser().parseFromString(
        strokegradients,
        "text/xml"
      )
...
;(function ($) {
  ...
  function Plugin(element, options) {
    ...
    this.carouselSlidesPathLength =
      this.options.numbersStyle === "circle" ? 471 : 200
  ...
  Plugin.prototype = {
    ...
    initFlicky() {
      ...
      imagesLoaded(this.element, () => {
        ...
        options.adaptiveHeight &&
          $(".flickity-viewport", this.element).css("transition", "height 0.3s")
        this.onImagesLoaded()
    ...
    events() {
      ...
      $("[data-toggle=tab]").on("hidden.bs.tab shown.bs.tab", (event) => {
        ...
        const $target = $(
          $($link.attr("href")),
          $link.parent().next(".lqd-tabs-content")
        )
    ...
    wrapItems() {
      ...
      if (
        ...
        $firstChild.hasClass("flickity-viewport-wrap")
      ) {
      ...
      $cells.each((i, cell) => {
        ...
        const $cellToSearch = cellHasInner
          ? $cell.find(".carousel-item-content")
          : $cell
        const hasOneChild =
          $cellToSearch.children().not("style, .vc_controls-container")
            .length === 1
        let classnames
        if ($cell.attr("class")) {
          if ($cell.hasClass("lqd-prod-item")) {
            classnames = $cell
              ...
              .filter(
                (classname) =>
                  classname !== "lqd-prod-item" && classname !== "product"
              )
          } else {
            classnames = $cell
              .attr("class")
        ...
        if ($cell.hasClass("carousel-item")) {
          middleAlignContent &&
            equalHeightCells &&
            !randomVerOffset &&
            $cell.addClass("align-items-center")
          ...
          $cell
            .wrap(
              `<div class="carousel-item ${
                hasOneChild ? "has-one-child" : ""
              } ${classnames && classnames.join(" ")} ${
                middleAlignContent && equalHeightCells && !randomVerOffset
                  ? "align-items-center"
                  : ""
              }" />`
            )
    ...
    setColumnWidth($cell, $cellChildren, $cellContentWrapper) {
      const width = $cellChildren.length
        ? $cellChildren.outerWidth()
        : $cellContentWrapper.outerWidth()
    ...
    carouselNav() {
      ...
      if (appendingSelector === "parent_row") {
        appendingSelector = liquidIsElementor
      ...
      let carouselId = this.options.carouselEl
        ? this.$element.attr("id")
        : this.$carouselContainer.attr("id")
      if (liquidIsElementor) {
        carouselId = `elementor-element elementor-element-${this.$element
          .closest(".elementor-element")
          .attr("data-id")}`
      }
      $.each($(this.$carouselContainer[0].classList), (i, className) => {
        if (className.indexOf("carousel-nav-") >= 0)
          carouselNavClassnames.push(className)
      })
      ...
      if (appendingSelector != null) {
        if ($appendingSelector.hasClass("vc_column_container")) {
          ...
          if (this.options.appendingBtnRel) {
            $carouselNav.appendTo(
              this.$carouselEl[this.options.appendingBtnRel](appendingSelector)
            )
          } else {
    ...
    carouselDots() {
      ...
      const carouselDots = $(
        `<div class="carousel-dots ${
          hasPageDotsFromOptions
            ? "carousel-dots-from-options"
            : "carousel-dots-to-hide"
        }"></div>`
      )
      let carouselId = carouselEl
      ...
      $.each($(this.$carouselContainer[0].classList), (i, className) => {
        if (className.indexOf("carousel-dots-") >= 0)
          carouselDotsClassnames.push(className)
      })
    ...
    carouselMobileDots(force = false) {
      ...
      const carouselId = carouselEl
        ? this.$carouselEl.attr("id")
        : this.$carouselContainer.attr("id")
      const mobileDotsClassnames = [carouselId]
      $.each($(this.$carouselContainer[0].classList), (i, className) => {
        if (className.indexOf("carousel-dots-mobile-") >= 0)
          mobileDotsClassnames.push(className)
      })
      const $dotsHolder = $(this.flickityData.pageDots.holder).clone(true)
      const $carouselMobileDots = $(
        `<div class="carousel-dots-mobile carousel-dots-style4 ${mobileDotsClassnames.join(
          " "
        )}"></div>`
      )
      ...
      this.flickityData.on("select", (i) =>
        this.carouselMobileDotsClasslist.call(this, dots[i], i)
      )
    },
    carouselMobileDotsClasslist(activeItem, activeItemIndex) {
      ...
      const inActives = [
        ...
        (inactiveItem, inactiveIndex) => activeItemIndex !== inactiveIndex
      )
    ...
    carouselDotsNumbers() {
      ...
      if (numbersStyle === "circle") {
        ...
        $svgMarkup = $(
          '<div class="lqd-carousel-slides-numbers d-inline-flex pos-rel inline-flex relative">' +
            ...
            '<path fill="none" stroke-dashoffset="' +
            this.carouselSlidesPathLength +
            '" stroke-dasharray="' +
            this.carouselSlidesPathLength +
            '" stroke-width="3" x="2" d="M75,150 C116.421356,150 150,116.421356 150,75 C150,33.5786438 116.421356,0 75,0 C33.5786438,0 0,33.5786438 0,75 C0,116.421356 33.5786438,150 75,150 Z"/>' +
            "</svg>" +
            "</div>"
        )
        ...
        $svgMarkup = $(
          '<div class="lqd-carousel-slides-numbers d-inline-flex pos-rel inline-flex relative lqd-carousel-numbers-line">' +
    ...
    addSlideNumbersToArrows() {
      ...
      this.$carouselTotalSlides = $(
        ...
        $(prevButtonEl).next(".lqd-carousel-slides")
      )
    ...
    createSlideNumbers(isZeroBased) {
      const totalSlides =
        (this.flickityData.slides.length < 10 && isZeroBased ? "0" : "") +
    ...
    changeSlidesNumbers() {
      ...
      const selectedNum = this.$carouselCurrentSlideInner
        .children("span")
        .eq(selectedIndex)[0]
    ...
    changeSlidesShape() {
      ...
      const slidesPercentage = Math.floor(
        (Math.abs(Math.floor(flickityData.x + flickityData.cursorPosition)) /
          Math.abs(Math.floor(flickityData.slidesWidth))) *
          100
      )
    ...
    fullwidthSide() {
      ...
      const viewportElOffsetRight =
        this.windowWidth - (elementWidth + viewportElOffset.left)
      ...
      let viewportElWrap = existingViewportWrap.length
      ...
      viewportElWrap.css({
        [margin]: !this.isRTL
          ? viewportElOffsetRight >= 0
            ? (viewportElOffsetRight - 1) * -1
            : viewportElOffsetRight - 1
          : viewportElOffset.left * -1,
        [padding]: !this.isRTL
          ? Math.abs(viewportElOffsetRight - 1)
          : Math.abs(viewportElOffset.left - 1),
      })
      this.flickityData.resize()
    },
    randomVerOffset() {
    ...
    setElementNavArrow() {
      ...
      const prevButton = this.flickityData.prevButton
        ? this.flickityData.prevButton.element
        : null
      const nextButton = this.flickityData.nextButton
    ...
    controllingCarousels() {
      ...
      if (
        ...
        const $controlledCarousels = $(
          controllingCarousels.map((carousel) =>
            $(carousel).children("[data-lqd-flickity]")
          )
        )
        $.each($controlledCarousels, (i, controlledCarousel) => {
          const $controlledCarousel = $(controlledCarousel)
          $controlledCarousel.imagesLoaded(() => {
            const controlledCarouselData = $controlledCarousel.data(
            ...
            if (controlledCarouselData) {
              controlledCarouselData.carouselInitPromise.then(() => {
                ...
                controlledCarouselData.carouselMobileDotsElement &&
                  controlledCarouselData.carouselMobileDotsElement.classList.add(
                    "hidden"
                  )
    ...
    doSomethingCrazyWithFilter() {
      ...
      const firstVisibleCell = this.flickityData.cells.filter((cell) =>
        $(cell.element).is(":visible")
      )[0]
      const firstCellWidth = firstVisibleCell.size.width
      const opacityVal = gsap.utils.normalize(
        -firstCellWidth,
        0,
        this.isRTL ? x + size.width : x
      )
      const rotationVal = gsap.utils.mapRange(
        0,
        -firstCellWidth,
        0,
        -100,
      ...
      const zVal = gsap.utils.mapRange(
        0,
        -firstCellWidth,
        0,
        -300,
    ...
    addFilterCounts($filterItem, filterValue) {
      const count =
        filterValue === "*"
          ...
          : $(filterValue, this.element).length
      const $counter = $(`
    ...
    filterAnimateStart(filterValue) {
      const visibleCells = this.getCellsArray().filter(
  ...
  $.fn[pluginName] = function (options) {
    return this.each(function () {
      ...
      const globalDisabledOnMobile = document.body.hasAttribute(
        "data-disable-carousel-onmobile"
      )
      if (
        liquidIsMobile() &&
        globalDisabledOnMobile &&
        !pluginOptions.forceEnableOnMobile
      ) {
...
;(function ($) {
  ...
  class Plugin {
    ...
    updateContainerHeight(index, diff) {
      ...
      let height = 0
      $.each(
        [$(msSlides[index - 1]), $(msSlides[index]), $(msSlides[index + 1])],
        (i, msSlide) => {
    ...
    customStyles(index, diff, lower) {
      ...
      if (!lower) {
        ...
        const $prevItem = $(msSlides[index - 1])
        const translateVal = Math.round(
          (Math.abs(diff) - $prevItem.outerHeight()) / -1
        )
        $nextItem.css({
          opacity: opacityVal,
        })
  ...
  class Plugin {
    ...
    _setStackStyle(direction) {
      ...
      this.items.forEach((item) =>
        item.classList.remove("is-first", "is-second", "is-third")
      )
      if (item1) {
...
jQuery(document).ready(function ($) {
  ...
  $(".lqd-carousel-stack")
    .filter((i, el) => !el.closest(".lqd-tabs-pane:not(.active)"))
    .liquidCarouselStack()
})
;(function ($) {
  "use strict"
    ...
    prepareitems() {
      ...
      if (items.length <= 2 && items.length >= 1) {
        ...
        for (let i = items.length; i <= 2; i++) {
          $(firstItem)
            .clone(true)
            .appendTo($(self.element).find(".carousel-items"))
        }
      }
    },
      ...
      const animationTimeline = gsap.timeline({
        ...
        onComplete: () => {
          $(items, element).removeClass(
            "is-moving was-top was-active was-bottom"
          )
          $(this.element).removeClass("items-moving")
        },
      })
      if (dir == "next") this.setActive(bottomItem, element)
      else if (dir == "prev") this.setActive(topItem, element)
...
;(function ($) {
  ...
  class Plugin {
      ...
      this.dropdownInfoPromise = new Promise((resolve) => {
        this.element.addEventListener("megamenu-position-applied", async () => {
          resolve({
            ...
            megamenuBoundingRect:
              this.megamenuBoundingRect ||
              (await this.getMegamenuBoundingRect()),
    ...
    setContainerWidth() {
      return fastdomPromised.mutate(() => {
        this.megamenuRowsWrap.style.width = `${
          this.columnsWidth - this.defaultSidePadding * 2
        }px`
      })
    }
    ...
    positioning() {
      ...
      const globalContainerDimensions = fastdom.measure(
      ...
      const menuItemisInGlobalContainerRange =
        elementOffsetLeft <= globalContainerWidth + globalContainerOffsetLeft
      let left = 0
      let right = 0
      let top = 0
      ...
      fastdomPromised.mutate(() => {
        if (!this.isFullwidth) {
          if (
            megamenuContainerWidth === globalContainerWidth &&
            menuItemisInGlobalContainerRange
          ) {
          ...
          if (menuItemisInGlobalContainerRange) {
            left =
              globalContainerOffsetLeft +
              (globalContainerWidth / 2 - megamenuContainerWidth / 2) -
          ...
          if (
            megamenuOffsetLeft + megamenuContainerWidth <
        ...
        if (this.isInCustomMenu) {
          ...
          if (
            ...
            megamenuOffsetTop + megamenuHeight
          ) {
          ...
          if (this.isFullwidth) {
            if (
              this.megamenuBoundingRect.left + megamenuContainerWidth >
              this.windowWidth
            ) {
              right =
                (this.windowWidth - (elementOffsetLeft + elementWidth)) * -1
  ...
  class Plugin {
    constructor(element, options) {
      ...
      this.DOM.$megamenuItems = this.DOM.$element
        ...
        .filter(
          (i, el) =>
            !$(el).parent().hasClass("lqd-mobile-main-nav") &&
            !$(el).parent().hasClass("lqd-menu-items-block")
        )
      this.DOM.$nonMegamenuItems = this.DOM.$megamenuItems
        .eq(0)
        .siblings()
        .not(".megamenu")
      this.dropdownsInfo = []
    ...
    eventListeners() {
      document.addEventListener("lqd-submenu-change", (e) => {
        ...
        if ($submenuParent?.length) {
          const megamenuIndex = this.DOM.$megamenuItems
            .get()
            .indexOf($submenuParent[0])
          if (megamenuIndex >= 0) {
            if (state === "show") {
    ...
    onMegamenuItemEnter(itemIndex) {
      ...
      if (!this.wasRevealed) {
        ...
        this.DOM.element.style.setProperty(
          ...
          megamenuBoundingRect.width
        )
        this.DOM.element.style.setProperty(
          "--lqd-megamenu-init-height",
          megamenuBoundingRect.height
        )
      } else {
      ...
      this.DOM.element.style.setProperty(
        "--lqd-megamenu-x",
        `${megamenuFinalPos.left || 0}px`
      )
      this.DOM.element.style.setProperty(
        "--lqd-megamenu-y",
        `${megamenuFinalPos.top || 0}px`
      )
    ...
    onMegamenuItemLeave(itemIndex) {
      ...
      this.DOM.$megamenuItems
        .eq(itemIndex)
        .addClass("lqd-megamenu-item-slide-out")
...
jQuery(document).ready(function ($) {
  $("[data-megamenu-slide]")
    .filter((i, el) => $(el).find(".megamenu").length)
    .liquidMegamenuSlide()
})
;(function ($) {
  "use strict"
...
;(function ($) {
  ...
  Plugin.prototype = {
    ...
    createMarkup: function () {
      ...
      if (formatWithCommas)
        counter.html(
          this.formatCounterAnimator(this.formatNumberWithCommas(counterVal))
        )
      else if (formatWithSpaces)
        counter.html(
          this.formatCounterAnimator(this.formatNumberWithSpaces(counterVal))
        )
      else counter.html(this.formatCounterAnimator(counterVal))
  ...
  class Plugin {
    ...
    initEvents() {
      ...
      if (!isMobile) {
        window.addEventListener(
          "mousemove",
          (ev) => (this.mousePos = liquidGetMousePos(ev))
        )
      }
    ...
    render() {
      ...
      this.brightness = Math.min(
        this.lineEq(1.75, 1, 140, 1, mouseDistance),
        1.5
      )
...
;(function ($) {
  ...
  class Plugin {
    ...
    initDatePicker() {
      const dateInputs = this.$element.find(
        ".date-picker, input.wpcf7-form-control[type=date]"
      )
    ...
    initSelect() {
      ...
      if (
        $selectElement.closest(
          ".checkout, table.cart, .elementor-widget-woocommerce-product-add-to-cart"
        ).length
      )
        return
        ...
        $selectElement.each((i, element) => {
          ...
          if (dropdownAppendTo) {
            ...
            if (dropdownAppendTo === "self") {
              ...
              $appendToEl = $(dropdownAppendTo, this.element).length
                ? $(dropdownAppendTo, this.element)
                : $(dropdownAppendTo)
            }
    ...
    initSpinner() {
      const splinners = this.$element.find(
        "input.spinner, input[type=number].qty"
      )
...
jQuery(document).ready(function ($) {
  $("form, .lqd-filter-dropdown, .widget")
    .not(
      '[name="chbs-form"], .frm-fluent-form, .hub-booking-form, .geodir-listing-search'
    )
    .liquidFormInputs()
})
;(function ($) {
  ...
  class Plugin {
    ...
    hover(e) {
      ...
      const transVals = {
        ...
        yRotationPercentage:
          this.calculateRotationPercentage(
            mouseOffsetInside.x,
            this.rect.width
          ) * -1,
...
jQuery(document).ready(function ($) {
  ...
  $("[data-hover3d]")
    .filter(
      (i, element) =>
        !$(element).closest(".lqd-tabs-pane").not(".active").length
    )
    .liquidHover3d()
})
;(function ($) {
  ...
  class Plugin {
    ...
    setTransformOrigin() {
      ...
      this.$items.each((i, element) => {
        ...
        $element.css({
          transformOrigin: `${parentDimensions.width / 2}px ${
...
;(function ($) {
  ...
  class Plugin {
    ...
    _animateDraggedHandle(event, xPosition) {
      ...
      var widthValue =
        ((leftValue - containerOffset) * 100) / this.containerWidth + "%"
      this.$dragElement
        .css("left", widthValue)
...
jQuery(document).ready(function ($) {
  ...
  const visibleElements = imageCopElements.filter(
    (i, element) =>
      !$(element)
        .closest(".lqd-tabs-pane, .e-n-tabs-content > .e-con")
        .not(".active, .e-active").length
  )
  const parentElementorTabs = imageCopElements.closest(
...
;(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      this.images = []
      ;[
        ...this.DOM.imgArrayContainer.querySelectorAll(".lqd-img-trail-img"),
      ].forEach((img) => this.images.push(new Img(img)))
    ...
    setThreshold(trigger) {
      if (this.options.threshold === "auto") {
        this.options.threshold =
          ($(trigger).outerWidth() / this.images.length - 1) / 1.75
      }
    }
    initEvents() {
    ...
    render() {
      ...
      if (distance > this.options.threshold) {
        ...
        if (
          ...
          this.imgPosition =
            this.imgPosition === 0 ? this.imagesTotal - 1 : this.imgPosition - 1
        }
        this.showNextImage()
...
;(function ($) {
  ...
  class Plugin {
    ...
    events() {
      $(document).on("lity:ready", (ev, instance) =>
        this.onReady.call(this, ev, instance)
      )
    ...
    boxModalPosition($modalWrapper, $opener) {
      ...
      $modalWrapper.css({
        left: offset,
        right:
          offset + modalWidth >= windowWidth ? windowWidth - offset : "auto",
      })
...
;(function ($) {
  ...
  class LqdGMap {
              ...
              for (var i = 0; i < this.options.markers.length; i++) {
                this.addMarker(
                  new google.maps.LatLng(
                    this.options.markers[i][0],
                    this.options.markers[i][1]
                  ),
                  map
                )
              }
    ...
    addMarker(pos, map) {
      if (
        this.options.marker_style === "image" ||
        this.options.marker_style === "default"
      ) {
        ...
        if (
          typeof google !== typeof undefined &&
          typeof google.maps !== typeof undefined
        ) {
...
;(function ($) {
  ...
  class Plugin {
    ...
    init() {
      const $collapsedParents = this.$element.parents(
        ".lqd-tabs-pane, .accordion-collapse"
      )
    ...
    setAlignMidStamps() {
      ...
      if (options.alignMid) {
        ...
        if (columnsCount >= 3) {
          gridSizer
            .clone()
            .insertBefore(items.eq(columnsCount - 1))
            .addClass("is-right")
    ...
    asSectionBg() {
      if (this.options.asBG) {
        ...
        const sectionContainerElement = liquidIsElementor
          ? parentSection.children(".elementor-container")
          : parentSection.children(".ld-container")
...
;(function ($) {
  ...
  class Plugin {
    ...
    async init() {
      ...
      this.handleResize()
      this.element.dispatchEvent(
        new CustomEvent("element-was-pinned", {
          bubbles: false,
        })
      )
    }
      ...
      return fastdomPromised.measure(() => {
        ...
        if (duration === "contentsHeight") {
          if (
            ...
            const $contentsContainer = this.spacerElement
        ...
        if (duration === "parent") {
          if (this.element.classList.contains("vc_column-inner")) {
            duration = `+=${
              this.element.closest(".ld-row").offsetHeight - this.rect.height
            }`
          } else {
            ...
            this.$element
              ...
              .each(
                (i, children) =>
                  (contentsHeight += $(children).outerHeight(true))
              )
...
;(function ($) {
  ...
  class Plugin {
    ...
    reveal(revealSettingsArg) {
      ...
      if (!liquidIsElementor) {
        this.revealer.style.background =
      ...
      animationSettings.duration = animationSettings_2.duration =
        revealSettings.duration
          ? revealSettings.duration / 1000
          : defaults.duration
      animationSettings.ease = animationSettings_2.ease =
        revealSettings.ease || defaults.ease
    ...
    doTheReveal() {
      const onCoverAnimations = this.options.revealSettings
        .onCoverAnimations || [
        {
          scale: 0.9,
        },
...
;(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      this.DOM.$element = $(element)
      this.DOM.$triggers =
        this.options.triggers === "self"
          ? this.DOM.$element
          : this.DOM.$element.find(this.options.triggers)
      this.DOM.$targets =
        this.options.targets === "self"
          ? this.DOM.$element
    ...
    init() {
      const { triggerHandlers, classname, offClassname, lazyLoadImgVid } =
        this.options
      this.DOM.$triggers.each((i, trigger) => {
...
;(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      this.$element = $(element)
      this.$triggerElement =
        this.options.triggerElement === "self"
          ? this.$element
    ...
    getHiddenElementChilds() {
      return this.$hiddenElementInner
        .children()
        .map((i, childElement) => childElement.parentElement)
    }
    ...
    moveElements() {
      const translateVal = this.options.alignMid
    ...
    createTimeline() {
      ...
      let translateVal = options.alignMid
        ? this.hiddenElementHeight / 2
        : this.hiddenElementHeight
      if (options.hiddenElementOnHover) {
        const elementHeight = this.$hiddenElementOnHover.outerHeight(true)
        translateVal = options.alignMid
          ? (this.hiddenElementHeight + elementHeight) / 2
...
;(function ($) {
  ...
  const headerStickyOptions =
    $liquidMainHeader.length &&
    $liquidMainHeader[0].getAttribute("data-sticky-options")
  const headerIsDynamicColor =
    headerStickyOptions &&
    JSON.parse(headerStickyOptions).dynamicColors === true
  ...
  class Plugin {
    constructor(element, options) {
      ...
      this.$pageNumbersStyle = $liquidBody.hasClass("lqd-stack-nums-style-1")
        ? "style1"
        : $liquidBody.hasClass("lqd-stack-nums-style-2")
        ? "style2"
        : ""
      this.element.style.setProperty(
        "--lqd-stack-animation-speed",
      ...
      LiquidSectionsDetails.getDetails().then((sections) => {
        const contentSections = sections.filter(
          (sec) => !sec.isInFooter && !sec.isInnerSection
        )
    ...
    moveTo(section) {
      let sec
      if (isNaN(section)) {
        sec = this.$sectionElements.filter(
          (i, el) => el.getAttribute("data-anchor") === section
        )
      } else {
    ...
    getActiveSection() {
      const $element = this.$sectionElements.filter((i, el) =>
        $(el).hasClass("active")
      )
    ...
    makeScrollable(section) {
      ...
      if (childrenHeight > sectionHeight) {
        const sectionPadding =
          parseInt($section.css("paddingTop"), 10) +
          parseInt($section.css("paddingBottom"), 10)
        $section.addClass("pp-scrollable")
        section.style.setProperty(
          "--lqd-section-height",
          `${childrenHeight + sectionPadding}px`
        )
      } else {
    ...
    scrollToAnchor() {
      ...
      var section = this.$sectionElements.filter(
        (i, el) => $(el).attr("data-anchor") === sectionAnchor
      )
      if (section.length > 0) {
    ...
    isMoving() {
      ...
      if (
        ...
        this.scrollDelay + parseFloat(this.options.scrollingSpeed) * 250
      ) {
        return true
      }
      return false
    ...
    hashChangeHandler() {
      ...
      if (sectionAnchor.length) {
        if (sectionAnchor && sectionAnchor !== this.lastScrolledDestiny) {
          const section = this.$sectionElements.filter(
            (i, el) => $(el).attr("data-anchor") === sectionAnchor
          )
          this.scrollPage(section)
        }
    ...
    mouseWheelHandler(e) {
      ...
      var horizontalDetection =
        typeof evt.wheelDeltaX !== "undefined" ||
        typeof evt.deltaX !== "undefined"
      var isScrollingVertically =
        Math.abs(evt.wheelDeltaX) < Math.abs(evt.wheelDelta) ||
      ...
      if (this.isReallyTouch(e)) {
        ...
        if (!this.isMoving()) {
          ...
          if (
            ...
            (this.$element.height() / 100) * this.options.touchSensitivity
          ) {
    ...
    addNavigationItem(i) {
      this.$ppNavCurrent
        .find("> span")
        .append(`<span>${i < 10 ? "0" + (i + 1) : i + 1}</span>`)
    ...
    build() {
      ...
      if ($liquidMainFooter.length && !window.frameElement) {
        let $toAppendFooter
        if (liquidIsElementor) {
          const $secWrap = $liquidContentsWrap.find(
            " > .elementor > .elementor-section-wrap"
          )
          $toAppendFooter = $secWrap.length
            ? $secWrap
            : $liquidContentsWrap.find(" > .elementor")
        } else {
        ...
        section.addEventListener("animationend", (event) => {
          if (event.target === section) {
            section.classList.remove(
              ...
              "lqd-stack-row-moving-up",
              "lqd-stack-row-moving-down"
            )
          }
        })
      })
    ...
    appendPageNumber() {
      let $pageNumber
      if (this.$pageNumbersStyle === "style1")
        $pageNumber = this.appendPageNumbersStyle1()
      if (this.$pageNumbersStyle === "style2")
        $pageNumber = this.appendPageNumbersStyle2()
    ...
    setPageNumbersStyle1(index) {
      const $currentPageNumber = this.$pageNumber.find(
        ".lqd-stack-page-number-current"
      )
      const $passedPageNumber = this.$pageNumber.find(
    ...
    addLuminosityClassnames(index) {
      fastdom.mutate(() => {
        const $elements = !headerIsDynamicColor
          ? $liquidBody
    ...
    afterRender() {
      ...
      const activeSectionIndex = this.$sectionElements
        .get()
    ...
    onLeave(index, nextIndex, direction) {
      ...
      if (
        !$destinationRow.hasClass("main-footer") &&
        !$originRow.hasClass("main-footer")
      ) {
    ...
    handleMainNavLocalScroll() {
      ...
      if (winHash) {
        ...
        this.$mainNavLocalScroll
          .closest(
            ".navbar-fullscreen, .navbar-collapse, .ld-module-dropdown, .mobile-navbar-collapse"
          )
          .collapse("hide")
      }
    }
  ...
  class Plugin {
    ...
    init() {
      ...
      this.DOM.$innerItems.each((i, innerItem) => {
        const scaleVal = distributor(
          i,
          this.DOM.$innerItems.get(i),
      ...
      this.DOM.$items.each((i, item) => {
        ScrollTrigger.create({
          trigger: item,
          start: `top-=${
            offset + i * spacer + parseInt(stickyHeaderHeight)
          } top`,
    ...
    getOffset() {
      ...
      const $stickySections = $liquidMainHeader
...
;(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      if (this.$activeItem.length && this.options.translateNav) {
        ...
        this.activeItemPosLeft = !this.isRTL
          ...
          : this.$tabNav.width() -
            (this.$activeItem.position().left + this.activeItemWidth) * -1
      }
    ...
    changeIds() {
      this.$navItems.each((i, navItem) => {
        ...
        navItem.setAttribute(
          "aria-controls",
          `${navItem.getAttribute("aria-controls")}-mobile`
        )
        this.element
          .querySelector(href)
          .setAttribute("id", `${href.replace("#", "")}-mobile`)
      })
    }
    init() {
    ...
    onShow(event) {
      ...
      if (translateNav) {
        ...
        this.activeItemPosLeft = !this.isRTL
          ? this.$activeItem.position().left
          : (this.$tabNav.width() -
              (this.$activeItem.position().left + this.activeItemWidth)) *
            -1
    ...
    onShown(event) {
      ...
      const offsetTop = liquidIsMobile()
        ? $(".lqd-tabs-content", this.$element).offset().top
        : this.$element.offset().top
      const stickyHeaderHeight =
        $("[data-sticky-header].is-stuck")?.outerHeight() || 0
      const link = event.target
      const dataTarget = link.getAttribute("data-bs-target")
      if (dataTarget) {
        const target = document.querySelector(dataTarget)
      ...
      if (
        ...
        (liquidIsMobile() &&
          (offsetTop > $liquidWindow.scrollTop() + window.innerHeight ||
            $liquidWindow.scrollTop() > offsetTop))
      ) {
        $("html, body")
          .stop()
          .animate(
    ...
    return this.each(function () {
      ...
      if (!$.data(this, "plugin_" + pluginName)) {
        $.data(
          this,
          "plugin_" + pluginName,
          new Plugin(this, inlineVidOptions, YTOptions)
        )
      }
    })
  }
  ...
  class Plugin {
    ...
    init() {
      ...
      preloadImages().then(() => {
        new RGBShiftEffect(
          this.element,
          this.element.querySelector("[data-hoverme]"),
          {
            strength: 3,
          }
        )
...
;(function ($) {
  ...
  class Plugin {
    ...
    animateElements() {
      ...
      gsap.fromTo(
        $currentExtrasChildElements.get(),
        {
          xPercent: -3,
          opacity: 0,
        },
...
;(function ($) {
  ...
  class Plugin {
    ...
    initSnickersBar($btn, data) {
      const cartItem = this.cartItems.find(
        (item) =>
          ...
          item.id === $btn.attr("data-product_id") ||
          item.id === $btn.attr("value")
      )
      if (cartItem) {
    ...
    createSnickersBar($btn, data) {
      ...
      $msg.text(
        $addningMsgTemp.text().replace(/\{\{itemName\}\}/, product_name)
      )
      $msgDone.text(
        $addedMsgTemp.text().replace(/\{\{itemName\}\}/, product_name)
      )
    ...
    onAddedToCart(frags, $btn) {
      ...
      if (cartItem) {
        ...
        !$snickersBarExtEl.children(".added_to_cart").length &&
          $snickersBarExtEl.append($btn.siblings(".added_to_cart").clone(true))
    ...
    hideSnickersBar(cartItem) {
      const { $snickersBarEl, snickersBarHeight, isVisible } = cartItem
      const timeOutVal = liquidIsElementor
    ...
    upadteSnickersBarsPos() {
      ...
      this.cartItems.forEach((item) => {
        ...
        item.$snickersBarEl.css(
          "transform",
          `translateY(${(this.snickersBarsHeight - heights) * -1}px)`
        )
      })
    }
    ...
    updateCartAmount(frags) {
      if (!frags) return
      const $headerCartAmount = this.DOM.$headerCart.find(
...
;(function ($) {
  ...
  let defaults = {
    ...
    itemsSelector:
      ":scope > .elementor-container > .elementor-column > .elementor-widget-wrap > .elementor-section, :scope > .e-container, :scope > .e-con",
  }
  class Plugin {
    constructor(element, options) {
      this._defaults = defaults
    ...
    async createScroller() {
      ...
      await fastdomPromised.mutate(() => {
        $(this.DOM.items).wrapAll(
          `<div class="${this.options.scroller.substring(1)} lqd-overlay" />`
        )
        this.DOM.scroller = this.DOM.element.querySelector(
    ...
    events() {
      ...
      this.DOM.scroller.addEventListener(
        "transitionend",
        this.onScrollerTransitionEnd
      )
    ...
    onElementMouseWheel(event) {
      ...
      if (
        (this.activeItem === 0 && this.isScrollingDown) ||
        (this.activeItem === this.DOM.items.length - 1 && this.isScrollingUp)
      ) {
        $("html, body").animate(
          {
            scrollTop:
    ...
    appendDots() {
      ...
      const $dotsWrapper = $(
        '<div class="lqd-section-scroll-dots d-flex flex-column pos-abs z-index-5" />'
      )
      this.itemsRects.forEach((rect, i) => {
        const $dot = $(
          `<div class="lqd-section-scroll-dot d-flex align-items-center justify-content-center border-radius-circle pos-rel text-center ${
            i === 0 ? "is-active" : ""
          }"><span></span></div>`
        )
    ...
    destroy() {
      ...
      this.DOM.scroller.removeEventListener(
        "transitionend",
        this.onScrollerTransitionEnd
      )
...
;(function ($) {
  ...
  const localscrollGlobalOffset =
    bodyLSOffset != null && bodyLSOffset !== "" ? parseInt(bodyLSOffset, 10) : 0
  ...
  class Plugin {
    constructor(element, options) {
      ...
      this.$element = $(element)
      this.$items =
        this.options.itemsSelector === "self"
          ? this.$element
    ...
    async getOffsets() {
      ...
      const $elements = !includeParentAsOffset
        ? $(offsetElements)
        : $(offsetElements).add(this.$element.parent())
      const promises = []
    ...
    getTargetsArray(itemIndex, item) {
      if (this.options.scrollBelowSection) {
        const $parentRow = liquidIsElementor
          ? this.$element
              .parents(".elementor-section, .e-container, .e-con")
              .last()
          : this.$element.parents(".vc_row, .vc_section").last()
        const $nextRow = liquidIsElementor
          ? $parentRow
              .nextAll(".elementor-section, .e-container, .e-con")
              .first()
    ...
    onClick(event) {
      const { offset, scrollBelowSection, scrollSpeed } = this.options
      let y = 0
      ...
      const processScroll = () => {
            ...
            this.targetsRects.forEach((rect) => {
              ...
              if (rect.y === objectPositionPercentage[0]) {
                let DistanceFromCenter = [
                  rect.y,
                  this.getVisibilityDistanceFromCenter(rect, ScrollPos),
                ]
                if (
        ...
        if (max !== this.lastItemPercent) {
          ...
          StagesPositionPercentage.forEach((objectPositionPercentage) => {
            ...
            if (
              objectPositionPercentage[1] === max &&
              objectPositionPercentage[0] !== this.CurrentPositionTop &&
              !this.fakePercent
            ) {
    ...
    checkIsInView(rect, ScrollPos = this.getScrollPos()) {
      return (
        (rect.y > ScrollPos[0] && rect.y < ScrollPos[1]) ||
        (rect.bottom > ScrollPos[0] && rect.bottom < ScrollPos[1]) ||
        (rect.y < ScrollPos[0] && rect.bottom > ScrollPos[1])
      )
    }
    getScrollPos() {
      let offsetTop
    ...
    getVisibilityDistanceFromCenter(
      targetRect,
    ...
    _addBgElement() {
      return fastdomPromised.mutate(() => {
        ...
        if (this.$element.hasClass("vc_column_container")) {
          $insertPlace = this.$element.find(
            "> .vc_column-inner > .row-bg-loader"
          )
        }
  ...
  class Plugin {
    ...
    _addImages() {
      $.each(this.imageArray, (i, { src, alt }) => {
        const $img = $(
          `<img class="invisible" src="${src}" alt="${
            alt || "Slideshow image"
          }"/>`
        )
        const $figure = $(
          `<figure class="ld-slideshow-figure" style="background-image: url(${src})" />`
        )
  ...
  class Plugin {
    ...
    _measure() {
      return fastdomPromised.measure(() => {
        ...
        this.fontInfo.elementFontFamily = styles.fontFamily
          .replace(/"/g, "")
          .replace(/'/g, "")
          .split(",")[0]
      ...
      return fastdomPromised.measure(() => {
        if (
          window.liquidCheckedFonts.find(
            (ff) => ff === this.fontInfo.fontFamilySlug
          )
        ) {
    ...
    _split() {
      ...
      const fancyHeadingInner =
        this.element.classList.contains("ld-fh-txt") &&
        this.element.querySelector(".ld-fh-txt-inner") != null
      const el = fancyHeadingInner
    ...
    _unitsOp() {
      return fastdomPromised.mutate(() => {
        for (const [splitType, splittedTextArray] of Object.entries(
          this.splittedTextList
        )) {
          if (splittedTextArray && splittedTextArray.length > 0) {
            splittedTextArray.forEach((splitElement, i) => {
              ...
              splitElement.style.setProperty(
                `--${splitType}-last-index`,
                splittedTextArray.length - 1 - i
              )
...
jQuery(document).ready(function ($) {
  const $elements = $("[data-split-text]").filter((i, el) => {
    ...
    const hasCustomAnimationParent = $el.closest(
    ...
    const webglSlideshowParent = $el.closest(
      "[data-lqd-webgl-slideshow]"
    ).length
    return (
      !isCustomAnimation &&
  ...
  class Plugin {
    ...
    _getInnerChildElements(elements) {
      ...
      let $elements = $(elements).map((i, element) => {
        const $element = $(element)
        if (
          $element.hasClass("vc_inner") ||
          $element.hasClass("vc_vc_row_inner")
        ) {
          return $element.find(".wpb_wrapper").children().get()
        } else if ($element.hasClass("row")) {
          return $element.find(".lqd-column").children().get()
        } else if (
          $element.hasClass("ld-slideelement-visible") ||
          $element.hasClass("ld-slideelement-hidden")
        ) {
      ...
      $.each($elements, (i, element) => {
        ...
        if (element.querySelector("[data-custom-animations]")) {
          return element
            .querySelectorAll("[data-custom-animations]")
            .forEach((el) => {
        ...
        if (
          element.classList.contains("vc_inner") ||
          element.classList.contains("vc_vc_row_inner")
        ) {
        ...
        if (
          ...
          if (
            ...
            element.classList.contains("vc_ld_button")
          ) {
    ...
    _initValues() {
      ...
      const initProps = !randomizeInitValues
        ? initValues
        : this._generateRandomValues(initValues)
      $animationTargets
        .css({
          transition: "none",
          transitionDelay: 0,
...
jQuery(document).ready(function ($) {
  const anims = $("[data-custom-animations]")
    .filter((i, element) => {
      const $element = $(element)
      const stackOptions =
        $liquidContents.length &&
        $liquidContents[0].getAttribute("data-stack-options")
      const stackDisableOnMobile =
        stackOptions && JSON.parse(stackOptions).disableOnMobile === true
      return (
        (!stackOptions ||
          (stackOptions &&
            stackDisableOnMobile &&
            (liquidIsMobile() ||
...
;(function ($) {
  ...
  class Plugin {
    ...
    build() {
      const promises = []
      const $customAnimationParent = this.$element.closest(
        "[data-custom-animations]"
      )
      const $customAnimationChild = this.$element.children(
        "[data-custom-animations]"
      )
      ...
      if ($customAnimationParent.length) {
        const data = $customAnimationParent.data(
          "plugin_liquidCustomAnimations"
        )
    ...
    setContainerWidth(index) {
      ...
      if (this.options.animationType === "list") {
        return (keywordContainer.style.width = `${Math.max(
          ...this.keywordsDimensions.map((dim) => parseInt(dim.width, 10))
        )}px`)
      }
    ...
    slideInNextKeyword() {
      ...
      const delay = this.isFirstItterate
    ...
    slideOutAciveKeyword() {
      ...
      const delay = this.isFirstItterate
        ...
        : this.options.delay
      $activeKeyword
        .removeClass("lqd-keyword-slide-in")
        .addClass("lqd-keyword-slide-out")
    ...
    buildListAnimation() {
      const duration = 2
      const visibleWords = parseInt(
        getComputedStyle(this.keywordsContainer).getPropertyValue(
          "--visible-words"
        ),
        10
      )
      const totalHeight = this.keywordsDimensions
        .map((dim) => dim.height)
        .reduce((prevVal, newVal) => prevVal + newVal, 0)
      const listHeight = this.keywordsDimensions
      ...
      this.$keywords
        ...
        .each((i, keyword) => {
          ...
          mainTimeline
            ...
            .add(
              `keyword-${i + 1}`,
              gsap.utils.mapRange(0, totalKeywords, 0, duration)(i)
            )
      ...
      const slideKeywordsInner = () => {
        gsap.set([this.keywordsInner, this.keywordsInnerClone], {
          "--current-keyword-height": `${
            (this.keywordsDimensions[currentKeyword - 1].height / 2) * -1
          }px`,
        })
      }
      slideKeywordsInner()
      const scrubTimeline = (from, to) => {
        if (wrapping) {
          return new gsap.timeline()
            .add(
              mainTimeline.tweenFromTo(from, duration, {
                duration: this.options.duration,
                ease: this.options.easing,
              })
            )
            .add(
              mainTimeline.tweenFromTo(0, to, {
                duration: this.options.duration,
                ease: this.options.easing,
      ...
      function animateTo() {
        ...
        slideKeywordsInner()
        currentKeyword =
          currentKeyword >= totalKeywords ? 1 : currentKeyword + 1
...
;(function ($) {
  ...
  class Plugin {
    ...
    initParallaxBG() {
      ...
      if (isSlideshowBg) {
        return slideshowBgPlugin.slideshowBgInitPromise.then(
          (slideshowPlugin) => {
    ...
    createParallaxBgMarkup() {
      ...
      parallaxContainer.setAttribute(
        "class",
    ...
    return this.each(function () {
      ...
      if (!$.data(this, "plugin_" + pluginName)) {
        ...
        $.data(
          this,
          "plugin_" + pluginName,
          new Plugin(this, pluginOptions, parallaxFrom, parallaxTo)
        )
      }
    })
  }
...
;(function ($) {
  ...
  class Plugin {
    ...
    addDelays() {
      const { elements, delayBetween, startDelay, delayType, reverse } =
        this.options
      if (elements) {
        const $elements = !reverse
          ? $(elements, this.element)
          : $(elements, this.element).get().reverse()
        $.each($elements, (i, element) => {
  ...
  class Plugin {
    ...
    getHiddenClasses() {
      ...
      if (parentColumn) {
        ...
        const classList = parentColumnClass
          .split(" ")
          .filter((cls) => cls.search("vc_hidden") >= 0)
    ...
    moveElement() {
      fastdom.mutate(() => {
        ...
        this.element.dispatchEvent(
          new CustomEvent("element-was-moved", {
            bubbles: false,
          })
        )
      })
...
;(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      if (this.options.waitForElementMove) {
        const $parentElementMove = $(this.DOM.element).closest(
          "[data-move-element]"
        )
        if ($parentElementMove.length) {
          return $parentElementMove
    ...
    getScrollingTarget() {
      ...
      switch (scrollingTarget) {
        ...
        default:
          ...
          let target = isLinkedToElement
            ? document.querySelector(scrollingTarget)
            : scrollingTarget
          if (target === "body") {
    ...
    initialTrigger() {
      ...
      if (!scale) {
        tween = gsap.to(indicatorEl, {
          [dir]:
            dir === "y"
              ? indicatorBase.offsetHeight
              : indicatorBase.offsetWidth,
          force3D: false,
        })
      } else {
        tween = gsap.fromTo(
    ...
    initMutationObserver() {
      ...
      stackData.stackInitPromise.then((plugin) => {
        ...
        const totalSections = [...plugin.$sectionElements].filter(
          (section) => !section.classList.contains("main-footer")
        ).length
        const indicatorBaseSize =
          dir === "y" ? indicatorBase.offsetHeight : indicatorBase.offsetWidth
        new MutationObserver(() => {
          const currentStackPage = document.body.getAttribute(
            "data-lqd-stack-page"
          )
          const indicatorTrans = gsap.utils.mapRange(
            1,
            totalSections,
            0,
            indicatorBaseSize,
  ...
  $(".nav-item-children-w-sub > div").on("click", () => {
    $(".nav-item-children-w-sub").toggleClass("active")
  })
})(jQuery)
jQuery(document).ready(function ($) {
