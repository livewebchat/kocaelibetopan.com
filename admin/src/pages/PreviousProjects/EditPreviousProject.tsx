...
type Props = {
  ...
  setPreviousProjectForEdit: Dispatch<
    SetStateAction<PreviousProject | undefined>
  >;
  fetchPreviousProjects: () => Promise<void>;
};

export const EditPreviousProject: React.FC<Props> = ({
  previousProjectForEdit,
  setPreviousProjectForEdit,
  fetchPreviousProjects,
}) => {
  const [title, setTitle] = useState(previousProjectForEdit.title);
    ...
    <>
      <div className="flex flex-col gap-9 fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] z-[1000] w-[90%] max-w-[500px] max-h-[90vh] overflow-y-auto">
        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          <div className="border-b border-stroke py-4 px-6.5 dark:border-strokedark">
            <div className="flex justify-between">
              <h3 className="font-medium text-black dark:text-white">
                <b>{previousProjectForEdit.title}</b> başlıklı slaytı düzenle
              </h3>
              <span
                className="cursor-pointer text-black-2 dark:text-white text-lg"
                onClick={() => setPreviousProjectForEdit(undefined)}
              >
                &times;
              </span>
            </div>
          ...
          <form
            onSubmit={handleEditPreviousProject}
                ...
                >
                  ...
                  {images ? (
                    ...
                    <div className="flex flex-col">
                      <img
                        className="h-full w-full aspect-video object-cover rounded"
                        src={`https://kocaelibetopan.com/uploads/${previousProjectForEdit.image}`}
                        alt={previousProjectForEdit.title}
                      />
                      <span className="mt-4">
                        Seçilen görsel:{' '}
                        <span className="text-graydark dark:text-white">
                          {previousProjectForEdit.title}
                        </span>
                      </span>
                    </div>
                  ) : (
